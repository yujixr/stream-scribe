...
@pytest.fixture
def settings() -> VADDetectionSettings:
    """デフォルトのVADDetectionSettings"""
    return VADDetectionSettings()


@pytest.fixture
def state_machine(settings: VADDetectionSettings) -> VadStateMachine:
...
class TestHysteresisThresholds:
    ...
    def test_uses_high_threshold_when_not_recording(
        self, state_machine: VadStateMachine, settings: VADDetectionSettings
    ) -> None:
        """録音していない時は高い閾値を使用"""
        # 開始閾値未満は音声としてカウントされない
        state_machine.process(settings.start_threshold - 0.01)
        assert state_machine.speech_chunks == 0

        # 開始閾値以上は音声としてカウント
        state_machine.process(settings.start_threshold)
    ...
    def test_uses_low_threshold_when_recording(
        ...
        """録音中は低い閾値を使用"""
        # まず録音を開始
        for _ in range(settings.min_speech_chunks):
            state_machine.process(settings.start_threshold + 0.1)
        assert state_machine.is_recording is True

        # 終了閾値と開始閾値の間は、録音中なら音声としてカウント
        prob_between = (settings.end_threshold + settings.start_threshold) / 2
    ...
    def test_below_end_threshold_counts_silence_when_recording(
        ...
        # 終了閾値未満
        state_machine.process(settings.end_threshold - 0.01)
...
class TestRecordingStart:
    ...
    def test_starts_recording_after_min_speech_chunks(
        ...
        """settings.min_speech_chunks後に録音開始"""
        high_prob = settings.start_threshold + 0.1

        # settings.min_speech_chunks - 1 チャンクを処理
        for _ in range(settings.min_speech_chunks - 1):
            action = state_machine.process(high_prob)
            assert action == VadAction.NONE
            assert state_machine.is_recording is False

        # settings.min_speech_chunks目で開始をトリガー
        action = state_machine.process(high_prob)
    ...
    def test_resets_speech_counter_on_silence(
        ...
        """無音で音声カウンターがリセットされる"""
        high_prob = settings.start_threshold + 0.1
        low_prob = settings.start_threshold - 0.1

        # 音声チャンクを蓄積
        for _ in range(settings.min_speech_chunks - 1):
            state_machine.process(high_prob)
        assert state_machine.speech_chunks == settings.min_speech_chunks - 1
...
class TestRecordingStop:
    ...
    def test_stops_recording_after_max_silence_chunks(
        ...
        """settings.max_silence_chunks後に録音終了"""
        high_prob = settings.start_threshold + 0.1
        low_prob = settings.end_threshold - 0.1
        ...
        # settings.max_silence_chunks - 1 無音チャンクを処理
        for _ in range(settings.max_silence_chunks - 1):
            action = state_machine.process(low_prob)
            assert action == VadAction.NONE
            assert state_machine.is_recording is True

        # settings.max_silence_chunks目で停止をトリガー
        action = state_machine.process(low_prob)
    ...
    def test_speech_during_silence_resets_silence_counter(
        ...
        high_prob = settings.start_threshold + 0.1
        medium_prob = settings.end_threshold + 0.05  # 終了閾値以上
        low_prob = settings.end_threshold - 0.1
        ...
        # 無音を蓄積
        for _ in range(settings.max_silence_chunks - 2):
            state_machine.process(low_prob)
        assert state_machine.silence_chunks == settings.max_silence_chunks - 2
    ...
    def test_state_reset_after_stop(
        ...
        for _ in range(settings.min_speech_chunks):
            state_machine.process(high_prob)
        for _ in range(settings.max_silence_chunks):
            state_machine.process(low_prob)
...
class TestVadModelReset:
    ...
    def test_triggers_reset_after_idle_silence(
        ...
        # 録音していない状態で待機中の無音を蓄積
        for _ in range(settings.idle_reset_chunks - 1):
            action = state_machine.process(low_prob)
            assert action == VadAction.NONE
    ...
    def test_no_reset_during_recording(
        self, state_machine: VadStateMachine, settings: VADDetectionSettings
    ) -> None:
        """録音中はリセットしない"""
        high_prob = settings.start_threshold + 0.1
        low_prob = settings.end_threshold + 0.05  # 終了閾値以上で録音継続
    ...
    def test_speech_resets_idle_counter(
        self, state_machine: VadStateMachine, settings: VADDetectionSettings
    ) -> None:
        """音声で待機中カウンターがリセットされる"""
        low_prob = settings.start_threshold - 0.1
        ...
        # 待機中の無音を蓄積
        for _ in range(settings.idle_reset_chunks // 2):
            state_machine.process(low_prob)
        assert state_machine.idle_silence_chunks > 0
...
class TestComplexScenarios:
    ...
    def test_intermittent_speech_pattern(
        self, state_machine: VadStateMachine, settings: VADDetectionSettings
    ) -> None:
        """短い休止を含む発話パターンをシミュレート"""
        high_prob = settings.start_threshold + 0.2
        medium_prob = settings.end_threshold + 0.1
        low_prob = settings.end_threshold - 0.1
        ...
        for _ in range(3):
            ...
            for _ in range(5):
                state_machine.process(medium_prob)
            # 短い休止
            for _ in range(settings.max_silence_chunks // 3):
                state_machine.process(low_prob)

        # まだ録音中のはず
        assert state_machine.is_recording is True

    def test_noise_spike_ignored(
        self, state_machine: VadStateMachine, settings: VADDetectionSettings
    ) -> None:
        """単発のノイズスパイクは録音を開始しない"""
        high_prob = settings.start_threshold + 0.3
        low_prob = settings.start_threshold - 0.2
    ...
    def test_full_conversation_cycle(
        ...
        actions.clear()
        for _ in range(settings.idle_reset_chunks):
            actions.append(state_machine.process(low_prob))
...
class TestEdgeCases:
    ...
    def test_exact_threshold_values(
        ...
        state_machine.process(settings.end_threshold)
        assert state_machine.silence_chunks == 0
